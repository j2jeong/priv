...
def get_training_transform():
    train_transform = [
        # albu.RandomBrightnessContrast(brightness_limit=0.25, contrast_limit=0.5, p=0.15),
        # albu.RandomBrightnessContrast(p=0.5),
        # albu.RandomCrop(height=256, width=256, always_apply=False, p=1.0),
...
def train_aug(img, mask):
    crop_aug = Compose([RandomScale(scale_list=[0.5, 0.75, 1.0, 1.25, 1.5], mode='value'),
                        SmartCropV1(crop_size=512, max_ratio=0.5,
...
# def train_aug(label, mask):
#     label, mask = np.array(label), np.array(mask)
#     aug = get_training_transform()(image=label.copy(), mask=mask.copy())
#     label, mask = aug['image'], aug['mask']
#     return label, mask
